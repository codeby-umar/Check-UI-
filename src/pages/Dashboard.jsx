...
const Dashboard = () => {
  ...
  const sendSMSResult = (res) => {
   const templateParams = {
    ...
    .catch((error) => {
      ...
      alert("Xatolik! Public Key yoki Template ID xato.");
    });
  };
  ...
  return (
    <div className="p-6 md:p-10 font-sans text-slate-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8">
          <h2 className="text-xl font-bold mb-8 flex items-center gap-2">
            <span className="w-2 h-6 bg-indigo-500 rounded-full"></span> Natijalar o'sishi
          </h2>
          <div className="h-90 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={[...results].reverse()}>
                ...
                <YAxis domain={[0, 100]} stroke="#cbd5e1" fontSize={12} tickLine={false} axisLine={false} />
                <Tooltip
        ...
        <div className="bg-white p-8 rounded-sm shadow-xl shadow-slate-200/50 border border-white flex flex-col items-center">
          <h2 className="text-xl font-bold mb-6 self-start tracking-tight">Activity</h2>
          <style>{`
      ...
      <h2 className="text-2xl font-black mt-16 mb-8 text-slate-800 tracking-tight">Recent attempts</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {results.map((res) => (
          <div key={res.id} className="group bg-white p-6 rounded-sm hover:shadow-indigo-100 transition-all duration-300 relative overflow-hidden">
            
            <button 
              onClick={() => sendSMSResult(res)}
              className="absolute -right-12 group-hover:right-2 top-2 bg-indigo-600 text-white p-2 rounded-lg transition-all duration-300 hover:bg-indigo-700 shadow-lg"
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="text-right">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
                  {res.date?.toDate().toLocaleDateString("uz-UZ", { day: "numeric", month: "short" })}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-end">
                <span className="text-3xl font-black text-slate-900">
                  {res.score}<span className="text-sm text-slate-400">%</span>
                </span>
                <span className={`text-[10px] font-bold ${res.score >= 70 ? "text-emerald-500" : "text-amber-500"}`}>
              ...
              <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden p-0.5">
                <div
                  className={`h-full rounded-full transition-all duration-1500 ease-out shadow-[0_0_10px_rgba(0,0,0,0.1)] ${
                    res.score >= 70 ? "bg-gradient-to-r from-emerald-400 to-teal-400" : "bg-gradient-to-r from-orange-400 to-amber-400"
                  }`}
...
const StatCard = ({ title, value, icon }) => (
  <div className="bg-white/60 backdrop-blur-md px-4 py-3 md:px-6 md:py-4 rounded-sm flex items-center gap-3">
    ...
    <div>
      <p className="text-[10px] md:text-xs text-slate-400 uppercase tracking-wider">{title}</p>
