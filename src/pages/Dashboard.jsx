...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (data.length > 0) {
        const avg =
          data.reduce((acc, curr) => acc + (Number(curr.score) || 0), 0) /
          data.length;
        setStats({
          ...
          growth: data[1]
            ? (
                ((data[0].score - data[1].score) / (data[1].score || 1)) *
                100
              ).toFixed(1)
            : 0,
        });
  ...
  const tileContent = ({ date, view }) => {
    if (view === "month") {
      const hasTest = results.some((res) => {
        const testDate = res.date?.toDate
          ? res.date.toDate()
          : new Date(res.date);
  ...
  return (
    <div className="h-screen w-full bg-[#0a0a0a] flex flex-col overflow-hidden text-white font-sans">
        ...
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <h1 className="text-5xl font-black italic tracking-tighter uppercase leading-none">
                ...
                <span className="text-[#B23DEB] not-italic">Analytics</span>
              </h1>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 px-6 py-3 flex items-center gap-4">
              <div className="text-right">
                <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">
                  {userData?.name || "Yuklanmoqda..."}
                </p>
                <p className="text-emerald-400 text-xs font-black uppercase tracking-tighter italic">
                  Active System
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 bg-[#111] border border-white/5 p-8 flex flex-col items-center">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] self-start text-gray-500 mb-6">
                Activity Log
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-12">
            {results.map((res) => (
              <div
                key={res.id}
                className="group bg-[#111] border border-white/5 p-8 hover:border-[#B23DEB]/30 transition-all duration-500 relative overflow-hidden"
              >
                <button
                ...
                <div className="flex justify-between items-end">
                  <span className="text-3xl font-black tracking-tighter italic">
                    {res.score}
                    <span className="text-xs text-gray-600 not-italic ml-1">
                      %
                    </span>
                  </span>
                  <span
                    className={`text-[8px] font-black uppercase tracking-widest ${res.score >= 70 ? "text-emerald-500" : "text-amber-500"}`}
                  >
      ...
      <style>{`
  /* 1. Ichki scrollbar rangini qora va minimalist qilish */
  ...
  .custom-scrollbar::-webkit-scrollbar-thumb {
    background: #1a1a1a; /* To'q qora rang */
    border-radius: 10px;
    border: 1px solid #222;
  }
  ...
  /* 2. Kalendarni to'liq qora dizaynga moslash */
  .custom-cal { 
  ...
  .react-calendar__navigation button { 
    color: #B23DEB !important; 
    font-size: 1rem !important;
    min-width: 30px !important;
  }
  .react-calendar__navigation button:enabled:hover,
  .react-calendar__navigation button:enabled:focus {
    background-color: rgba(178, 61, 235,) !important;
  }
  .react-calendar__tile--now { 
    background: rgba(178, 61, 235, 0.1) !important; 
    color: #B23DEB !important; 
    font-weight: bold;
  }
...
const MetricCard = ({ title, value, sub, icon, color, trend }) => (
  <div className="bg-[#111] border border-white/5 p-6 hover:border-[#B23DEB]/30 transition-all duration-500 group relative">
    <div className="flex justify-between items-start mb-6">
      ...
      <div
        className={`px-2 py-1 text-[9px] font-black ${parseFloat(trend) >= 0 ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}
      >
        {trend}%
      </div>
    </div>
    ...
    <p className="text-[9px] text-gray-600 mt-4 opacity-40 font-bold tracking-widest uppercase">
      {sub}
    </p>
  </div>
);
